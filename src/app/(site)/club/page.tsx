    ...
    <div className="relative overflow-hidden bg-black">
      ...
      <section className="relative flex min-h-[80vh] items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image
            src={content.clubPage?.hero?.imageSrc || "/uploads/FRIDAYYY.png"}
            alt={content.clubPage?.hero?.title || "Azzurri Club & Lounge"}
            fill
            priority
        ...
        <div className="relative z-10 mx-auto flex max-w-4xl flex-col items-center px-4 text-center mt-20">
          <FadeIn>
            <h1 className="heading-font text-5xl font-medium tracking-tight text-[#EFD077] md:text-7xl">
              {content.clubPage?.hero?.title || "Azzurri Club & Lounge"}
            </h1>
          </FadeIn>
          <FadeIn delay={120}>
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-16 px-6 lg:flex-row lg:gap-24">
          <FadeIn className="w-full lg:w-1/2">
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-zinc-900 shadow-2xl border border-white/5">
              <Image
                src={content.clubPage?.about?.imageSrc || "/uploads/event-2.jpg"}
            ...
            <div className="space-y-10">
              <div className="space-y-4">
                <h2 className="heading-font text-4xl font-medium tracking-tight text-[#EFD077] md:text-5xl">
                  {content.clubPage?.about?.title || "Elevate Your Night"}
                </h2>
                <div className="h-0.5 w-16 bg-azzurri-blue" />
              </div>

              <div className="space-y-8">
                {content.clubPage?.about?.paragraphs && content.clubPage.about.paragraphs.length > 0 ? (
                  content.clubPage.about.paragraphs.map((para, i) => (
                    <p key={i} className={`leading-relaxed font-light ${i === 0 ? 'text-xl lg:text-2xl text-zinc-300' : 'text-base lg:text-lg text-zinc-400'}`}>
                      {para}
                    </p>
                  ))
                ) : (
